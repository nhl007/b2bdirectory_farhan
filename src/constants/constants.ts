export const accountTypeOptions = [
  { label: "NDIS Provider", value: "NDIS Provider" },
  {
    label: "B2B Provider (Services for NDIS Providers)",
    value: "B2B Provider (Services for NDIS Providers)",
  },
];

export const selectOptions = [
  {
    label: "Support",
    options: [
      {
        value: "Support Worker",
        label: "Support Worker",
      },
      {
        value: "Accessing the Community",
        label: "Accessing the Community",
      },
      {
        value: "Nurse",
        label: "Nurse",
      },
      {
        value: "Mentor",
        label: "Mentor",
      },
      {
        value: "Social Worker",
        label: "Social Worker",
      },
    ],
  },
  {
    label: "Allied Health (Therapies)",
    options: [
      {
        value: "Art Therapy",
        label: "Art Therapy",
      },
      {
        value: "Counselling",
        label: "Counselling",
      },
      {
        value: "Developmental Educator",
        label: "Developmental Educator",
      },
      {
        value: "Dietician",
        label: "Dietician",
      },
      {
        value: "Key Worker",
        label: "Key Worker",
      },
      {
        value: "Music Therapy",
        label: "Music Therapy",
      },
      {
        value: "Exercise Physiology",
        label: "Exercise Physiology",
      },
      {
        value: "Occupational Therapy",
        label: "Occupational Therapy",
      },
      {
        value: "Physiotherapy",
        label: "Physiotherapy",
      },
      {
        value: "Podiatry",
        label: "Podiatry",
      },
      {
        value: "Psychology",
        label: "Psychology",
      },
      {
        value: "Psychotherapy",
        label: "Psychotherapy",
      },
      {
        value: "Social Worker",
        label: "Social Worker",
      },
      {
        value: "Speech Therapy",
        label: "Speech Therapy",
      },
      {
        value: "Therapy Assistant",
        label: "Therapy Assistant",
      },
      {
        value: "Other (Sand, Animal, Play)",
        label: "Other (Sand, Animal, Play)",
      },
    ],
  },
  {
    label: "Plan Support",
    options: [
      {
        value: "Plan Manager",
        label: "Plan Manager",
      },
      {
        value: "Support Coordinator",
        label: "Support Coordinator",
      },
      {
        value: "Specialist Support Coordinator",
        label: "Specialist Support Coordinator",
      },
      {
        value: "Recovery Coach (PRS)",
        label: "Recovery Coach (PRS)",
      },
    ],
  },
  {
    label: "Skills Development",
    options: [
      {
        value: "Support in Employment",
        label: "Support in Employment",
      },
      {
        value: "Life Skills",
        label: "Life Skills",
      },
      {
        value: "Mentoring",
        label: "Mentoring",
      },
      {
        value: "Travel Training",
        label: "Travel Training",
      },
    ],
  },
  {
    label: "Home Maintenance",
    options: [
      { label: "Cleaning", value: "Cleaning" },
      { label: "Gardening", value: "Gardening" },
      { label: "Yard Maintenance", value: "Yard Maintenance" },
      {
        label: "Handyperson and Repairs",
        value: "Handyperson and Repairs",
      },
    ],
  },

  {
    label: "Housing",
    options: [
      { label: "ILO", value: "ILO" },
      {
        label: "Specialist Disability Accommodation (SDA)",
        value: "Specialist Disability Accommodation (SDA)",
      },
      {
        label: "Medium Term Accommodation",
        value: "Medium Term Accommodation",
      },
      {
        label: "Respite/Short Term Accommodation",
        value: "Respite/Short Term Accommodation",
      },
      {
        label: "Supported Independent Living (SIL)",
        value: "Supported Independent Living (SIL)",
      },
    ],
  },

  {
    label: "Social, Health and Wellbeing",
    options: [
      {
        label: "Social Programs and Activities",
        value: "Social Programs and Activities",
      },
      { label: "Personal Training", value: "Personal Training" },
      {
        label: "Family and Peer Support Groups",
        value: "Family and Peer Support Groups",
      },
    ],
  },

  {
    label: "Children",
    options: [
      {
        label: "Early Intervention and Children",
        value: "Early Intervention and Children",
      },
    ],
  },

  {
    label: "Employment",
    options: [
      {
        label: "Finding and Keeping a Job",
        value: "Finding and Keeping a Job",
      },
      { label: "SLES", value: "SLES" },
    ],
  },

  {
    label: "Equipment",
    options: [{ label: "Disability Aids", value: "Disability Aids" }],
  },

  {
    label: "Equipment",
    options: [
      {
        value: "Disability Aids",
        label: "Disability Aids",
      },
    ],
  },
];

export const selectOptionsB2B = [
  {
    label: "Accounting and Financial Services",
    options: [
      { value: "Bookkeeping", label: "Bookkeeping" },
      { value: "Tax Preparation", label: "Tax Preparation" },
      { value: "Payroll Management", label: "Payroll Management" },
    ],
  },
  {
    label: "Insurance Services",
    options: [
      { value: "Business Insurance", label: "Business Insurance" },
      { value: "Professional Liability", label: "Professional Liability" },
    ],
  },
  {
    label: "Legal Services",
    options: [
      { value: "Contract Law", label: "Contract Law" },
      { value: "Employment Law", label: "Employment Law" },
    ],
  },
  {
    label: "Digital Marketing",
    options: [
      { value: "Social Media Management", label: "Social Media Management" },
      {
        value: "SEO and Online Advertising",
        label: "SEO and Online Advertising",
      },
    ],
  },
  {
    label: "Marketing Consulting",
    options: [
      { value: "Marketing Strategy", label: "Marketing Strategy" },
      { value: "Market Research", label: "Market Research" },
    ],
  },
  {
    label: "IT Support and Services",
    options: [
      { value: "Software Support", label: "Software Support" },
      { value: "Hardware Maintenance", label: "Hardware Maintenance" },
      { value: "Managed IT Services", label: "Managed IT Services" },
    ],
  },
  {
    label: "Cybersecurity and Data Protection",
    options: [
      { value: "Data Security Solutions", label: "Data Security Solutions" },
      { value: "Privacy Compliance", label: "Privacy Compliance" },
    ],
  },
  {
    label: "Software Solutions for NDIS Providers",
    options: [
      {
        value: "Client Management Systems",
        label: "Client Management Systems",
      },
      {
        value: "Custom Software Development",
        label: "Custom Software Development",
      },
    ],
  },
  {
    label: "Web and Digital Development",
    options: [
      {
        value: "Website Development and Design",
        label: "Website Development and Design",
      },
      {
        value: "Digital Accessibility Solutions",
        label: "Digital Accessibility Solutions",
      },
    ],
  },
  {
    label: "Recruitment and Staffing",
    options: [
      {
        value: "Specialized Staff Recruitment",
        label: "Specialized Staff Recruitment",
      },
      {
        value: "Temporary Staffing Solutions",
        label: "Temporary Staffing Solutions",
      },
    ],
  },
  {
    label: "HR Consulting",
    options: [
      { value: "Employee Relations", label: "Employee Relations" },
      {
        value: "Organisational Development",
        label: "Organisational Development",
      },
    ],
  },
  {
    label: "Professional Training and Development",
    options: [
      { value: "Staff Training Programs", label: "Staff Training Programs" },
      {
        value: "Leadership and Management Development",
        label: "Leadership and Management Development",
      },
    ],
  },
  {
    label: "Administrative Support",
    options: [
      { value: "Virtual Assistance", label: "Virtual Assistance" },
      {
        value: "Clerical and Administrative Services",
        label: "Clerical and Administrative Services",
      },
    ],
  },
  {
    label: "Transportation Services",
    options: [
      { value: "Vehicle Fleet Management", label: "Vehicle Fleet Management" },
      { value: "Transport Coordination", label: "Transport Coordination" },
    ],
  },
  {
    label: "Management Consulting",
    options: [
      { value: "Operational Efficiency", label: "Operational Efficiency" },
      { value: "Strategic Planning", label: "Strategic Planning" },
    ],
  },
  {
    label: "Health and Safety Consulting",
    options: [
      { value: "Workplace Safety", label: "Workplace Safety" },
      {
        value: "Risk Assessment and Management",
        label: "Risk Assessment and Management",
      },
    ],
  },
  {
    label: "NDIS Compliance and Audit Preparation Consulting",
    options: [
      {
        value: "Audit Readiness and Support",
        label: "Audit Readiness and Support",
      },
      {
        value: "Compliance Strategy and Policy Development",
        label: "Compliance Strategy and Policy Development",
      },
      {
        value: "Quality Management Systems",
        label: "Quality Management Systems",
      },
      {
        value: "Staff Training for NDIS Compliance",
        label: "Staff Training for NDIS Compliance",
      },
    ],
  },
];

export const statesOptions = [
  { label: "New South Wales (NSW)", value: "New South Wales (NSW)" },
  { label: "Queensland (QLD)", value: "Queensland (QLD)" },
  { label: "South Australia (SA)", value: "South Australia (SA)" },
  { label: "Tasmania (TAS)", value: "Tasmania (TAS)" },
  { label: "Victoria (VIC)", value: "Victoria (VIC)" },
  { label: "Western Australia (WA)", value: "Western Australia (WA)" },
  {
    label: "Australian Capital Territory (ACT)",
    value: "Australian Capital Territory (ACT)",
  },
  { label: "Northern Territory (NT)", value: "Northern Territory (NT)" },
];

export const selectLanguages = [
  { label: "AusLan", value: "AusLan" },
  { label: "English", value: "English" },
  { label: "Mandarin Chinese", value: "Mandarin Chinese" },
  { label: "Hindi", value: "Hindi" },
  { label: "Spanish", value: "Spanish" },
  { label: "French", value: "French" },
  { label: "Arabic", value: "Arabic" },
  { label: "Bengali", value: "Bengali" },
  { label: "Russian", value: "Russian" },
  { label: "Portuguese", value: "Portuguese" },
  { label: "Urdu", value: "Urdu" },
  { label: "Indonesian", value: "Indonesian" },
  { label: "German", value: "German" },
  { label: "Japanese", value: "Japanese" },
  { label: "Swahili", value: "Swahili" },
  { label: "Turkish", value: "Turkish" },
  { label: "Tamil", value: "Tamil" },
  { label: "Vietnamese", value: "Vietnamese" },
  { label: "Yoruba", value: "Yoruba" },
  { label: "Italian", value: "Italian" },
  { label: "Telugu", value: "Telugu" },
  { label: "Thai", value: "Thai" },
  { label: "Javanese", value: "Javanese" },
  { label: "Korean", value: "Korean" },
  { label: "Persian", value: "Persian" },
  { label: "Malayalam", value: "Malayalam" },
  { label: "Hausa", value: "Hausa" },
  { label: "Ukrainian", value: "Ukrainian" },
  { label: "Amharic", value: "Amharic" },
  { label: "Farsi", value: "Farsi" },
  { label: "Polish", value: "Polish" },
  { label: "Malay", value: "Malay" },
  { label: "Yoruba", value: "Yoruba" },
  { label: "Xhosa", value: "Xhosa" },
  { label: "Pashto", value: "Pashto" },
  { label: "Kannada", value: "Kannada" },
  { label: "Nepali", value: "Nepali" },
];

export const selectDeliveryOptions = [
  { label: "In Person", value: "In Person" },
  { label: "Mobile", value: "Mobile" },
  { label: "Online/Telehealth", value: "Online/Telehealth" },
];

export const genderOfAttendanceOptions = [
  { label: "Male", value: "Male" },
  { label: "Female", value: "Female" },
  { label: "Non-Binary", value: "Non-Binary" },
];

export const agesSupportedOptions = [
  {
    label: "Early Childhood (0-7 years)",
    value: "Early Childhood (0-7 years)",
  },
  { label: "Children (7-17 years)", value: "Children (7-17 years)" },
  { label: "Young People (18-21 years)", value: "Young People (18-21 years)" },
  { label: "Adults (22-59 years)", value: "Adults (22-59 years)" },
  { label: "Mature Age (60+ years)", value: "Mature Age (60 years)" },
];

export const selectPaymentOptions = [
  { label: "NDIS", value: "NDIS" },
  { label: "Private (non-NDIS)", value: "Private (non-NDIS)" },
  { label: "Medicare", value: "Medicare" },
  { label: "Age care", value: "Age Care" },
];

export const disabilitiesExperienceOptions = [
  { label: "Behavioral", value: "Behavioral" },
  { label: "DD", value: "DD" },
  { label: "OCD", value: "OCD" },
  { label: "ADHD", value: "ADHD" },
  { label: "Early Intervention", value: "Early Intervention" },
  { label: "Chronic Medical Conditions", value: "Chronic Medical Conditions" },
  { label: "divHearing Loss", value: "divHearing Loss" },
  { label: "Intellectual/Developmental", value: "Intellectual/Developmental" },
  { label: "Intellectual", value: "Intellectual" },
  { label: "Down Syndrome", value: "Down Syndrome" },
  { label: "Developmental Delay", value: "Developmental Delay" },
  { label: "Neurological", value: "Neurological" },
  { label: "Stroke", value: "Stroke" },
  { label: "divNeurological/Epilepsy", value: "divNeurological/Epilepsy" },
  { label: "MS", value: "MS" },
  { label: "ABI", value: "ABI" },
  { label: "Physical", value: "Physical" },
  { label: "divSpinal Cord Injury", value: "divSpinal Cord Injury" },
  { label: "divMuscular Dystrophy", value: "divMuscular Dystrophy" },
  { label: "Cerebral Palsy", value: "Cerebral Palsy" },
  { label: "Psychosocial/Mental Health", value: "Psychosocial/Mental Health" },
  { label: "Vision Impairment", value: "Vision Impairment" },
];

export const popularServicesOptions = [
  "Plan Manager",
  "Support Coordinator",
  "Support Worker",
  "Occupational Therapy",
  "Phycology",
  "Physiotherapy",
  "Positive Behavior Support Plan",
  "Key Worker",
];

export const complexNeedsSupportedOptions = [
  { label: "24hr Care", value: "24hr Care" },
  { label: "2:1 Support", value: "21 Support" },
  { label: "Behaviors of concern", value: "Behaviors of concern" },
  { label: "Catheter", value: "Catheter" },
  { label: "Colostomy bag", value: "Colostomy bag" },
  { label: "Diabetes", value: "Diabetes" },
  { label: "Medical administration", value: "Medical administration" },
  { label: "PEG feeding", value: "PEG feeding" },
  { label: "Registered nurse", value: "Registered nurse" },
  { label: "Trachea care", value: "Trachea care" },
  { label: "Wound care", value: "Wound care" },
];

export const otherProviderSkillsOptions = [
  { label: "Absconding", value: "Absconding" },
  { label: "Advocacy", value: "Advocacy" },
  { label: "After School Support", value: "After School Support" },
  { label: "Age Care", value: "Age Care" },
  { label: "Build Technology Skills", value: "Build Technology Skills" },
  { label: "CALD Support", value: "CALD Support" },
  { label: "Challenging Behaviors", value: "Challenging Behaviors" },
  { label: "Crisis Accommodation", value: "Crisis Accommodation" },
  { label: "Crisis Management", value: "Crisis Management" },
  { label: "Domestic Violence", value: "Domestic Violence" },
  { label: "Drug and Alcohol", value: "Drug and Alcohol" },
  { label: "Hospital Transition", value: "Hospital Transition" },
  { label: "Justice", value: "Justice" },
  { label: "Learn to Drive", value: "Learn to Drive" },
  { label: "LGBTQIA+", value: "LGBTQIA" },
  { label: "Mental Health", value: "Mental Health" },
  { label: "NDIS Applications", value: "NDIS Applications" },
  { label: "NDIS Plan Reassessments", value: "NDIS Plan Reassessments" },
  { label: "Palliative Care", value: "Palliative Care" },
  { label: "Public Guardian", value: "Public Guardian" },
  { label: "Youth Mentoring", value: "Youth Mentoring" },
];
